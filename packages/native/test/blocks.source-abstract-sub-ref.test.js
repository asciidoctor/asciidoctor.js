...
`
      ...
      assert.equal(block.getAttr('style'), 'source')
      assert.equal(block.getAttr('cloaked-context'), 'fenced_code')
      assert.equal(block.getAttr('language'), null)
      const output = await convertStringToEmbedded(input)
      assertCss(output, '.listingblock', 1)
      assertCss(output, '.listingblock pre code', 1)
      assertCss(output, '.listingblock pre code:not([class])', 1)
    })
...
`
      ...
      assert.equal(block.context, 'listing')
      assert.equal(block.getAttr('style'), 'source')
      assert.equal(block.getAttr('cloaked-context'), 'literal')
      assert.equal(block.getAttr('language'), null)
      const output = await convertStringToEmbedded(input)
      assertCss(output, '.listingblock', 1)
      assertCss(output, '.listingblock pre', 1)
      assertCss(output, '.listingblock pre code', 1)
      assertCss(output, '.listingblock pre code[data-lang]', 0)
    })
...
`
      ...
      assert.equal(block.getAttr('style'), 'source')
      assert.equal(block.getAttr('cloaked-context'), 'literal')
      assert.equal(block.getAttr('language'), 'js')
      const output = await convertStringToEmbedded(input)
      assertCss(output, '.listingblock', 1)
      assertCss(output, '.listingblock pre', 1)
      assertCss(output, '.listingblock pre code', 1)
      assertCss(output, '.listingblock pre code[data-lang]', 1)
    })
  })
...
`
      ...
      assert.equal(block.getAttr('reftext'), null)
