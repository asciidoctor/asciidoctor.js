...
describe('Substitutions', () => {
  describe('Macros', () => {
    test('a single-line link macro should be interpreted as a link', async () => {
      ...
      assert.equal(await para.subMacros(para.source), '<a href="/home.html" class="bare">/home.html</a>')
    })
    ...
    test('a single-line link macro with text should be interpreted as a link', async () => {
      ...
      assert.equal(await para.subMacros(para.source), '<a href="/home.html">Home</a>')
    })
    ...
    test('a mailto macro should be interpreted as a mailto link', async () => {
      ...
      assert.equal(await para.subMacros(para.source), '<a href="mailto:doc.writer@asciidoc.org">doc.writer@asciidoc.org</a>')
    })
    ...
    test('a mailto macro with text should be interpreted as a mailto link', async () => {
      ...
      assert.equal(await para.subMacros(para.source), '<a href="mailto:doc.writer@asciidoc.org">Doc Writer</a>')
    })
    ...
    test('a mailto macro with text and subject should be interpreted as a mailto link', async () => {
      ...
      assert.equal(await para.subMacros(para.source), '<a href="mailto:doc.writer@asciidoc.org?subject=Pull%20request">Doc Writer</a>')
    })
    ...
    test('a mailto macro with text, subject and body should be interpreted as a mailto link', async () => {
      ...
      assert.equal(await para.subMacros(para.source), '<a href="mailto:doc.writer@asciidoc.org?subject=Pull%20request&amp;body=Please%20accept%20my%20pull%20request">Doc Writer</a>')
    })
    ...
    test('a mailto macro with subject and body only should use e-mail as text', async () => {
      ...
      assert.equal(await para.subMacros(para.source), '<a href="mailto:doc.writer@asciidoc.org?subject=Pull%20request&amp;body=Please%20accept%20my%20pull%20request">doc.writer@asciidoc.org</a>')
    })
    ...
    test('a mailto macro supports id and role attributes', async () => {
      ...
      assert.equal(await para.subMacros(para.source), '<a href="mailto:doc.writer@asciidoc.org" id="contact" class="icon">doc.writer@asciidoc.org</a>')
    })
    ...
    test('should recognize inline email addresses', async () => {
      ...
      for (const input of emails) {
        ...
        assert.equal(await para.subMacros(para.source), `<a href="mailto:${input}">${input}</a>`)
      }
    })
    ...
    test('should recognize inline email address containing an ampersand', async () => {
      ...
      assert.equal(await para.applySubs(para.source), '<a href="mailto:bert&amp;ernie@sesamestreet.com">bert&amp;ernie@sesamestreet.com</a>')
    })
    ...
    test('should recognize inline email address surrounded by angle brackets', async () => {
      ...
      assert.equal(await para.applySubs(para.source), '&lt;<a href="mailto:doc.writer@asciidoc.org">doc.writer@asciidoc.org</a>&gt;')
    })

    test('should ignore escaped inline email address', async () => {
      const para = await blockFromString(`${BACKSLASH}doc.writer@asciidoc.org`)
      assert.equal(await para.subMacros(para.source), 'doc.writer@asciidoc.org')
    })

    test('a single-line raw url should be interpreted as a link', async () => {
      const para = await blockFromString('http://google.com')
      assert.equal(await para.subMacros(para.source), '<a href="http://google.com" class="bare">http://google.com</a>')
    })
    ...
    test('a single-line raw url with text should be interpreted as a link', async () => {
      ...
      assert.equal(await para.subMacros(para.source), '<a href="http://google.com">Google</a>')
    })

    test('a multi-line raw url with text should be interpreted as a link', async () => {
      const para = await blockFromString('http://google.com[Google\nHomepage]')
      assert.equal(await para.subMacros(para.source), '<a href="http://google.com">Google\nHomepage</a>')
    })
    ...
    test('a single-line raw url with attribute as text should be interpreted as a link with resolved attribute', async () => {
      ...
      assert.equal(await para.subMacros(para.subAttributes(para.source)), '<a href="http://google.com">Google Homepage</a>')
    })
    ...
    test('should not resolve an escaped attribute in link text', async () => {
      ...
      for (const [uri, macro] of Object.entries(cases)) {
        ...
        para.document.attributes['google_homepage'] = 'Google Homepage'
        assert.equal(await para.subMacros(para.subAttributes(para.source)), `<a href="${uri}">{google_homepage}</a>`)
      }
    })
    ...
    test('a single-line escaped raw url should not be interpreted as a link', async () => {
      ...
      assert.equal(await para.subMacros(para.source), 'http://google.com')
    })
    ...
    test('a comma separated list of links should not include commas in links', async () => {
      ...
      assert.equal(await para.subMacros(para.source), '<a href="http://foo.com" class="bare">http://foo.com</a>, <a href="http://bar.com" class="bare">http://bar.com</a>, <a href="http://example.org" class="bare">http://example.org</a>')
    })

    test('a single-line image macro should be interpreted as an image', async () => {
      const para = await blockFromString('image:tiger.png[]')
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><img src="tiger.png" alt="tiger"></span>')
    })
    ...
    test('should replace underscore and hyphen with space in generated alt text for an inline image', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><img src="tiger-with-family_1.png" alt="tiger with family 1"></span>')
    })
    ...
    test('an image macro with SVG image and text should be interpreted as an image with alt text', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><img src="tiger.svg" alt="Tiger"></span>')
    })
    ...
    test('an image macro with an interactive SVG image and alt text should be converted to an object element', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><object type="image/svg+xml" data="images/tiger.svg"><span class="alt">Tiger</span></object></span>')
    })
    ...
    test('an image macro with an interactive SVG image, fallback and alt text should be converted to an object element', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><object type="image/svg+xml" data="images/tiger.svg"><img src="images/tiger.png" alt="Tiger"></object></span>')
    })
    ...
    test('an image macro with an inline SVG image should be converted to an svg element', async () => {
      ...
      const result = normalizeTagSpaces(await para.subMacros(para.source))
    ...
    test('an image macro with an inline SVG image should be converted to an svg element even when data-uri is set', async () => {
      ...
      assert.match(normalizeTagSpaces(await para.subMacros(para.source)), /<svg\s[^>]*width="100">/)
    })
    ...
    test('an image macro with an SVG image should not use an object element when safe mode is secure', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><img src="images/tiger.svg" alt="Tiger"></span>')
    })
    ...
    test('a single-line image macro with text containing escaped square bracket should be interpreted as an image with alt text', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><img src="tiger.png" alt="[Another] Tiger"></span>')
    })
    ...
    test('a single-line image macro with text and dimensions should be interpreted as an image with alt text and dimensions', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><img src="tiger.png" alt="Tiger" width="200" height="100"></span>')
    })
    ...
    test('a single-line image macro with text and dimensions should be interpreted as an image with alt text and dimensions in docbook', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<inlinemediaobject><imageobject><imagedata fileref="tiger.png" contentwidth="200" contentdepth="100"/></imageobject><textobject><phrase>Tiger</phrase></textobject></inlinemediaobject>')
    })
    ...
    test('a single-line image macro with scaledwidth attribute should be supported in docbook', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<inlinemediaobject><imageobject><imagedata fileref="tiger.png" width="25%"/></imageobject><textobject><phrase>Tiger</phrase></textobject></inlinemediaobject>')
    })
    ...
    test('a single-line image macro with scaled attribute should be supported in docbook', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<inlinemediaobject><imageobject><imagedata fileref="tiger.png" scale="200"/></imageobject><textobject><phrase>Tiger</phrase></textobject></inlinemediaobject>')
    })
    ...
    test('should pass through role on image macro to DocBook output', async () => {
      ...
      const result = await para.subMacros(para.source)
    ...
    test('a single-line image macro with text and link should be interpreted as a linked image with alt text', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><a class="image" href="http://en.wikipedia.org/wiki/Tiger"><img src="tiger.png" alt="Tiger"></a></span>')
    })
    ...
    test('an inline image macro with link should be interpreted as a linked image in docbook', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<link xl:href="http://www.apache.org/licenses/LICENSE-2.0"><inlinemediaobject><imageobject><imagedata fileref="apache%20license%202_0.png"/></imageobject><textobject><phrase>Apache License 2.0</phrase></textobject></inlinemediaobject></link>')
    })
    ...
    test('a single-line image macro with text and link to self should be interpreted as a self-referencing image with alt text', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><a class="image" href="img/tiger.png"><img src="img/tiger.png" alt="Tiger"></a></span>')
    })
    ...
    test('an inline image macro with text and link to self should be interpreted as a self-referencing image in docbook', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<link xl:href="img/tiger.png"><inlinemediaobject><imageobject><imagedata fileref="img/tiger.png"/></imageobject><textobject><phrase>Tiger</phrase></textobject></inlinemediaobject></link>')
    })
    ...
    test('should link to data URI if value of link attribute is self and inline image is embedded', async () => {
      ...
      const output = normalizeTagSpaces(await para.subMacros(para.source))
    ...
    test('rel=noopener should be added to an image with a link that targets the _blank window', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><a class="image" href="http://en.wikipedia.org/wiki/Tiger" target="_blank" rel="noopener"><img src="tiger.png" alt="Tiger"></a></span>')
    })
    ...
    test('rel=noopener should be added to an image with a link that targets a named window when the noopener option is set', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><a class="image" href="http://en.wikipedia.org/wiki/Tiger" target="name" rel="noopener"><img src="tiger.png" alt="Tiger"></a></span>')
    })
    ...
    test('rel=nofollow should be added to an image with a link when the nofollow option is set', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><a class="image" href="http://en.wikipedia.org/wiki/Tiger" rel="nofollow"><img src="tiger.png" alt="Tiger"></a></span>')
    })
    ...
    test('a multi-line image macro with text and dimensions should be interpreted as an image with alt text and dimensions', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image"><img src="tiger.png" alt="Another Awesome Tiger" width="200" height="100"></span>')
    })
    ...
    test('an inline image macro with a url target should be interpreted as an image', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), 'Beware of the <span class="image"><img src="http://example.com/images/tiger.png" alt="tiger"></span>.')
    })
    ...
    test('an inline image macro with a float attribute should be interpreted as a floating image', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="image right"><img src="http://example.com/images/tiger.png" alt="tiger"></span> Beware of the tigers!')
    })
    ...
    test('should propagate id attribute on inline image', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span id="ruby-logo" class="image"><img src="ruby.png" alt="Ruby logo"></span> is the Ruby logo')
    })
    ...
    test('should propagate id attribute on inline image and use alt text as reftext when converting to DocBook', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<inlinemediaobject xml:id="ruby-logo"><imageobject><imagedata fileref="ruby.png"/></imageobject><textobject><phrase>Ruby logo</phrase></textobject></inlinemediaobject> is the Ruby logo')
    })
    ...
    test('should prepend value of imagesdir attribute to inline image target if target is relative path', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), 'Beware of the <span class="image"><img src="./images/tiger.png" alt="tiger"></span>.')
    })

    test('should not prepend value of imagesdir attribute to inline image target if target is absolute path', async () => {
      const para = await blockFromString('Beware of the image:/tiger.png[tiger].', { attributes: { imagesdir: './images' } })
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), 'Beware of the <span class="image"><img src="/tiger.png" alt="tiger"></span>.')
    })
    ...
    test('should match an inline image macro if target contains a space character', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), 'Beware of the <span class="image"><img src="big%20cats.png" alt="big cats"></span> around here.')
    })
    ...
    test('an icon macro should be interpreted as an icon if icons are enabled', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="icon"><img src="./images/icons/github.png" alt="github"></span>')
    })
    ...
    test('an icon macro should be interpreted as alt text if icons are disabled', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="icon">[github&#93;</span>')
    })
    ...
    test('should not mangle icon with link if icons are disabled', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="icon"><a class="image" href="https://github.com">[github&#93;</a></span>')
    })
    ...
    test('should not mangle icon inside link if icons are disabled', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<a href="https://github.com"><span class="icon">[github&#93;</span> GitHub</a>')
    })
    ...
    test('an icon macro should be interpreted as a font-based icon when icons=font', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="icon"><i class="fa fa-github"></i></span>')
    })
    ...
    test('an icon macro with a size should be interpreted as a font-based icon with a size when icons=font', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="icon"><i class="fa fa-github fa-4x"></i></span>')
    })
    ...
    test('an icon macro with flip should be interpreted as a flipped font-based icon when icons=font', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="icon"><i class="fa fa-shield fa-fw fa-flip-horizontal"></i></span>')
    })
    ...
    test('an icon macro with rotate should be interpreted as a rotated font-based icon when icons=font', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="icon"><i class="fa fa-shield fa-fw fa-rotate-90"></i></span>')
    })
    ...
    test('an icon macro with a role and title should be interpreted as a font-based icon with a class and title when icons=font', async () => {
      ...
      assert.equal(normalizeTagSpaces(await para.subMacros(para.source)), '<span class="icon red"><i class="fa fa-heart" title="Heart me"></i></span>')
    })
    ...
    test('should use the imagesdir attribute on the node when resolving the icon path', async () => {
      ...
      const iconUri = await icon.iconUri('wave')
    ...
    test('a single-line footnote macro should be registered and output as a footnote', async () => {
      ...
      assert.equal(await para.subMacros(para.source), 'Sentence text <sup class="footnote">[<a id="_footnoteref_1" class="footnote" href="#_footnotedef_1" title="View footnote.">1</a>]</sup>.')
    ...
    test('an escaped closing square bracket in a footnote should be unescaped when converted', async () => {
      ...
      assert.equal(await para.subMacros(para.source), '<sup class="footnote">[<a id="_footnoteref_1" class="footnote" href="#_footnotedef_1" title="View footnote.">1</a>]</sup>.')
      assert.equal(para.document.catalog.footnotes.length, 1)
      const footnote = para.document.catalog.footnotes[0]
    ...
    test('a footnote macro may contain a link macro', async () => {
      ...
      assert.equal(await para.subMacros(para.source), 'Share your code. <sup class="footnote">[<a id="_footnoteref_1" class="footnote" href="#_footnotedef_1" title="View footnote.">1</a>]</sup>')
      assert.equal(para.document.catalog.footnotes.length, 1)
    ...
    test('a footnote macro may contain a shorthand xref', async () => {
      ...
      assert.equal(await para.subMacros(para.source), 'text <sup class="footnote">[<a id="_footnoteref_1" class="footnote" href="#_footnotedef_1" title="View footnote.">1</a>]</sup>')
    ...
    test('should increment index of subsequent footnote macros', async () => {
      ...
      assert.equal(await para.subMacros(para.source), 'Sentence text <sup class="footnote">[<a id="_footnoteref_1" class="footnote" href="#_footnotedef_1" title="View footnote.">1</a>]</sup>. Sentence text <sup class="footnote">[<a id="_footnoteref_2" class="footnote" href="#_footnotedef_2" title="View footnote.">2</a>]</sup>.', 'should have both footnotes')
    ...
    test('a footnoteref macro with id and single-line text should be registered and output as a footnote', async () => {
      ...
      assert.equal(await para.subMacros(para.source), 'Sentence text <sup class="footnote" id="_footnote_ex1">[<a id="_footnoteref_1" class="footnote" href="#_footnotedef_1" title="View footnote.">1</a>]</sup>.')
      assert.equal(para.document.catalog.footnotes.length, 1)
      const footnote = para.document.catalog.footnotes[0]
      assert.equal(footnote.index, 1)
    ...
    test('a footnoteref macro with id should refer to footnoteref with same id', async () => {
      ...
      assert.equal(await para.subMacros(para.source), 'Sentence text <sup class="footnote" id="_footnote_ex1">[<a id="_footnoteref_1" class="footnote" href="#_footnotedef_1" title="View footnote.">1</a>]</sup>. Sentence text <sup class="footnoteref">[<a class="footnote" href="#_footnotedef_1" title="View footnote.">1</a>]</sup>.')
      assert.equal(para.document.catalog.footnotes.length, 1)
      const footnote = para.document.catalog.footnotes[0]
      assert.equal(footnote.index, 1)
      assert.equal(footnote.id, 'ex1')
    ...
    test('normal substitutions are performed on an index term macro', async () => {
      ...
      for (const macro of ['indexterm:[*Tigers*]', '(((*Tigers*)))']) {
        const para = await blockFromString(`${sentence}${macro}`)
        const output = await para.applySubs(para.source)
        assert.equal(output, sentence)
      }
    })
    ...
    test('should honor secondary and tertiary index terms when primary index term is quoted and contains equals sign', async () => {
      ...
      for (const macro of ['indexterm:["name=value",variable,assignment]', '(((name=value,variable,assignment)))']) {
        ...
        const output = (await para.subMacros(para.source)).replace(/\n/g, '')
    ...
    describe('Button macro', () => {
      test('btn macro', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<b class="button">Save</b>')
      })
      ...
      test('btn macro that spans multiple lines', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<b class="button">Rebase and merge</b>')
      })
      ...
      test('btn macro for docbook backend', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<guibutton>Save</guibutton>')
      })
    })
    ...
    describe('Keyboard macro', () => {
      ...
      test('kbd macro with single key, docbook backend', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<keycap>F3</keycap>')
      })

      test('kbd macro with key combination', async () => {
        const para = await blockFromString('kbd:[Ctrl+Shift+T]', { attributes: { experimental: '' } })
        assert.equal(await para.subMacros(para.source), '<span class="keyseq"><kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>T</kbd></span>')
      })
      ...
      test('kbd macro with key combination that spans multiple lines', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<span class="keyseq"><kbd>Ctrl</kbd>+<kbd>T</kbd></span>')
      })
      ...
      test('kbd macro with key combination, docbook backend', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<keycombo><keycap>Ctrl</keycap><keycap>Shift</keycap><keycap>T</keycap></keycombo>')
      })
      ...
      test('kbd macro with key combination delimited by plus containing a comma key', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<span class="keyseq"><kbd>Ctrl</kbd>+<kbd>,</kbd></span>')
      })
      ...
      test('kbd macro with key combination delimited by commas containing a plus key', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<span class="keyseq"><kbd>Ctrl</kbd>+<kbd>+</kbd>+<kbd>Shift</kbd></span>')
      })
      ...
      test('kbd macro looks for delimiter beyond first character', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<kbd>,te</kbd>')
      })
    ...
    describe('Menu macro', () => {
      test('should process menu using macro syntax', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<b class="menuref">File</b>')
      })
      ...
      test('should process menu for docbook backend', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<guimenu>File</guimenu>')
      })
      ...
      test('should process multiple menu macros in same line', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<b class="menuref">File</b> and <b class="menuref">Edit</b>')
      })
      ...
      test('should process menu with menu item using macro syntax', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<span class="menuseq"><b class="menu">File</b>&#160;<b class="caret">&#8250;</b> <b class="menuitem">Save As&#8230;</b></span>')
      })
      ...
      test('should process menu macro that spans multiple lines', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<span class="menuseq"><b class="menu">Preferences</b>&#160;<b class="caret">&#8250;</b> <b class="menuitem">Compile\non\nSave</b></span>')
      })
      ...
      test('should unescape escaped closing bracket in menu macro', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<span class="menuseq"><b class="menu">Preferences</b>&#160;<b class="caret">&#8250;</b> <b class="menuitem">Compile [on] Save</b></span>')
      })
      ...
      test('should process menu with menu item using macro syntax when fonts icons are enabled', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<span class="menuseq"><b class="menu">Tools</b>&#160;<i class="fa fa-angle-right caret"></i> <b class="submenu">More Tools</b>&#160;<i class="fa fa-angle-right caret"></i> <b class="menuitem">Extensions</b></span>')
      })
      ...
      test('should process menu with menu item for docbook backend', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<menuchoice><guimenu>File</guimenu> <guimenuitem>Save As&#8230;</guimenuitem></menuchoice>')
      })
      ...
      test('should process menu with menu item in submenu using macro syntax', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<span class="menuseq"><b class="menu">Tools</b>&#160;<b class="caret">&#8250;</b> <b class="submenu">Project</b>&#160;<b class="caret">&#8250;</b> <b class="menuitem">Build</b></span>')
      })
      ...
      test('should process menu with menu item in submenu for docbook backend', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<menuchoice><guimenu>Tools</guimenu> <guisubmenu>Project</guisubmenu> <guimenuitem>Build</guimenuitem></menuchoice>')
      })
      ...
      test('inline menu syntax should not match closing quote of XML attribute', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<span class="xmltag">&lt;node&gt;</span><span class="classname">r</span>')
      })
      ...
      test('should process menu macro with items containing multibyte characters', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<span class="menuseq"><b class="menu">视图</b>&#160;<b class="caret">&#8250;</b> <b class="submenu">放大</b>&#160;<b class="caret">&#8250;</b> <b class="menuitem">重置</b></span>')
      })
      ...
      test('should process a menu macro with a target that begins with a character reference', async () => {
        ...
        assert.equal(await para.subMacros(para.source), '<span class="menuseq"><b class="menu">&#8942;</b>&#160;<b class="caret">&#8250;</b> <b class="submenu">More Tools</b>&#160;<b class="caret">&#8250;</b> <b class="menuitem">Extensions</b></span>')
      })
