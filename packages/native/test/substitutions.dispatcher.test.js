...
describe('Substitutions', () => {
  describe('Dispatcher', () => {
    ...
    test('should not drop trailing blank lines when performing substitutions', async () => {
      const para = await blockFromString('[%hardbreaks]\nthis\nis\n-> {program}')
      para.lines.push('')
      para.lines.push('')
    ...
    test('should expand subs passed to expandSubs', async () => {
      ...
      assert.deepEqual(para.expandSubs(['normal']), ['specialcharacters', 'quotes', 'attributes', 'replacements', 'macros', 'post_replacements'])
    })
