...
describe('Substitutions', () => {
  describe('Quotes', () => {
    test('single-line double-quoted string', async () => {
      ...
      assert.equal(para.subQuotes(para.source), '&#8220;a few quoted words&#8221;')

      para = await blockFromString('"`a few quoted words`"')
      assert.equal(para.subQuotes(para.source), '&#8220;a few quoted words&#8221;')

      para = await blockFromString('"`a few quoted words`"', { backend: 'docbook' })
      assert.equal(para.subQuotes(para.source), '<quote role="double">a few quoted words</quote>')
    })
    ...
    test('escaped single-line double-quoted string', async () => {
      ...
      assert.equal(para.subQuotes(para.source), `${BACKSLASH}"\`a few quoted words\`"`)
    })

    test('multi-line double-quoted string', async () => {
      let para = await blockFromString("``a few\nquoted words''", { attributes: { 'compat-mode': '' } })
      assert.equal(para.subQuotes(para.source), '&#8220;a few\nquoted words&#8221;')
    ...
    test('double-quoted string with inline single quote', async () => {
      ...
      assert.equal(para.subQuotes(para.source), "&#8220;Here's Johnny!&#8221;")
    ...
    test('double-quoted string around monospaced text', async () => {
      ...
      assert.equal(para.applySubs(para.source), '&#8220;`E=mc<sup>2</sup>` is the solution!&#8221;')

      para = await blockFromString('"```E=mc^2^`` is the solution!`"')
      assert.equal(para.applySubs(para.source), '&#8220;<code>E=mc<sup>2</sup></code> is the solution!&#8221;')
    })
    ...
    test('single-line single-quoted string', async () => {
      ...
      para = await blockFromString("'`a few quoted words`'", { backend: 'docbook' })
      assert.equal(para.subQuotes(para.source), '<quote role="single">a few quoted words</quote>')
    })
    ...
    test("single-quoted string with inline single quote", async () => {
      ...
      assert.equal(para.subQuotes(para.source), "&#8216;That isn't what I did.&#8217;")
    ...
    test('constrained marked string should not match entity references', async () => {
      ...
      assert.equal(para.subQuotes(para.source), '111 <mark>mark a</mark> 222 &#8220;quote a&#8221; 333 <mark>mark b</mark> 444')
    })

    test('single-line unconstrained marked string', async () => {
      const para = await blockFromString('##--anything goes ##')
      assert.equal(para.subQuotes(para.source), '<mark>--anything goes </mark>')
    })
    ...
    test('does not recognize attribute list with left square bracket on formatted text', async () => {
      ...
      assert.equal(para.subQuotes(para.source), 'key: [ <strong>before <span class="redacted">redacted</span> after</strong> ]')
    })
    ...
    test('constrained strong string containing an asterisk and multibyte word chars', async () => {
      ...
      assert.equal(para.subQuotes(para.source), '<strong>黑*眼圈</strong>')
    })
    ...
    test('single-line constrained quote variation emphasized string', async () => {
      ...
      assert.equal(para.subQuotes(para.source), '<em>a few emphasized words</em>')
    })
    ...
    test('escaped single-line constrained quote variation emphasized string', async () => {
      ...
      assert.equal(para.subQuotes(para.source), '_a few emphasized words_')
    })
    ...
    test('escaped single quoted string', async () => {
      ...
      assert.equal(para.subQuotes(para.source), `${BACKSLASH}'a few emphasized words'`)
    })

    test('multi-line constrained emphasized quote variation string', async () => {
      const para = await blockFromString('_a few\nemphasized words_')
      assert.equal(para.subQuotes(para.source), '<em>a few\nemphasized words</em>')
    })
    ...
    test('single-quoted string containing an emphasized phrase', async () => {
      ...
      assert.equal(para.subQuotes(para.source), "&#8216;I told him, <em>Just go for it!</em>&#8217;")

      para = await blockFromString("'`I told him, 'Just go for it!'`'")
      assert.equal(para.subQuotes(para.source), "&#8216;I told him, 'Just go for it!'&#8217;")
    })
    ...
    test("escaped single-quotes inside emphasized words are restored", async () => {
      ...
      assert.equal(para.applySubs(para.source), "<em>Here's Johnny!</em>")

      para = await blockFromString(`'Here${BACKSLASH}'s Johnny!'`)
      assert.equal(para.applySubs(para.source), "'Here's Johnny!'")
    })
    ...
    test('single-line constrained monospaced string', async () => {
      ...
      assert.equal(para.applySubs(para.source), '<code>a few &lt;{monospaced}&gt; words</code>')
    ...
    test('single-line constrained monospaced string with role', async () => {
      ...
      assert.equal(para.applySubs(para.source), '<code class="input">a few &lt;{monospaced}&gt; words</code>')
    ...
    test('escaped single-line constrained monospaced string', async () => {
      ...
      assert.equal(para.applySubs(para.source), '`a few &lt;monospaced&gt; words`')
    ...
    test('escaped single-line constrained monospaced string with role', async () => {
      ...
      assert.equal(para.applySubs(para.source), '[input]`a few &lt;monospaced&gt; words`')
    ...
    test('escaped role on single-line constrained monospaced string', async () => {
      ...
      assert.equal(para.applySubs(para.source), '[input]<code>a few &lt;monospaced&gt; words</code>')
    ...
    test('escaped role on escaped single-line constrained monospaced string', async () => {
      ...
      assert.equal(para.applySubs(para.source), `${BACKSLASH}[input]\`a few &lt;monospaced&gt; words\``)
    ...
    test('should ignore role that ends with transitional role on constrained monospace span', async () => {
      ...
      assert.equal(para.applySubs(para.source), '<code class="foox-">leave it alone</code>')
    })
    ...
    test('escaped single-line constrained monospace string with forced compat role', async () => {
      ...
      assert.equal(para.applySubs(para.source), '[x-]`leave it alone`')
    })
    ...
    test('single-line unconstrained strong chars', async () => {
      ...
      assert.equal(para.subQuotes(para.source), '<strong>Git</strong>Hub')
    })
    ...
    test('multi-line unconstrained strong chars', async () => {
      ...
      assert.equal(para.subQuotes(para.source), '<strong>G\ni\nt\n</strong>Hub')
    })
    ...
    test('single-line constrained monospaced chars', async () => {
      ...
      assert.equal(para.subQuotes(para.source), 'call <code>save()</code> to persist the changes')

      para = await blockFromString('call [x-]+save()+ to persist the changes')
      assert.equal(para.applySubs(para.source), 'call <code>save()</code> to persist the changes')
    ...
    test('single-line constrained monospaced chars with role', async () => {
      ...
      assert.equal(para.subQuotes(para.source), 'call <code class="method">save()</code> to persist the changes')

      para = await blockFromString('call [method x-]+save()+ to persist the changes')
      assert.equal(para.applySubs(para.source), 'call <code class="method">save()</code> to persist the changes')
    ...
    test('escaped single-line constrained monospaced chars', async () => {
      ...
      assert.equal(para.subQuotes(para.source), 'call +save()+ to persist the changes')
    ...
    test('escaped single-line constrained monospaced chars with role', async () => {
      ...
      assert.equal(para.subQuotes(para.source), 'call [method]+save()+ to persist the changes')
    ...
    test('escaped role on single-line constrained monospaced chars', async () => {
      ...
      assert.equal(para.subQuotes(para.source), 'call [method]<code>save()</code> to persist the changes')
    ...
    test('escaped role on escaped single-line constrained monospaced chars', async () => {
      ...
      assert.equal(para.subQuotes(para.source), `call ${BACKSLASH}[method]+save()+ to persist the changes`)
    ...
    test('single-line unconstrained monospaced chars', async () => {
      ...
      assert.equal(para.subQuotes(para.source), 'Git<code>Hub</code>')

      para = await blockFromString('Git[x-]++Hub++')
      assert.equal(para.applySubs(para.source), 'Git<code>Hub</code>')
    ...
    test('escaped single-line unconstrained monospaced chars', async () => {
      ...
      assert.equal(para.subQuotes(para.source), 'Git++Hub++')
    ...
    test('multi-line unconstrained monospaced chars', async () => {
      ...
      assert.equal(para.subQuotes(para.source), 'Git<code>\nH\nu\nb</code>')

      para = await blockFromString('Git[x-]++\nH\nu\nb++')
      assert.equal(para.applySubs(para.source), 'Git<code>\nH\nu\nb</code>')
    ...
    test('single-line superscript chars', async () => {
      ...
      assert.equal(para.subQuotes(para.source), "x<sup>2</sup> = x * x, e = mc<sup>2</sup>, there's a 1<sup>st</sup> time for everything")
    })

    test('escaped single-line superscript chars', async () => {
      const para = await blockFromString(`x${BACKSLASH}^2^ = x * x`)
      assert.equal(para.subQuotes(para.source), 'x^2^ = x * x')
    })

    test('does not match superscript across whitespace', async () => {
      const para = await blockFromString('x^(n\n-\n1)^')
      assert.equal(para.subQuotes(para.source), para.source)
    })
    ...
    test('allow spaces in superscript if spaces are inserted using an attribute reference', async () => {
      ...
      assert.equal(para.applySubs(para.source), 'Night <sup>A poem by Jane Kondo</sup>.')
    })
    ...
    test('does not match adjacent superscript chars', async () => {
      ...
      assert.equal(para.subQuotes(para.source), 'a ^^ b')
    })
    ...
    test('does not confuse superscript and links with blank window shorthand', async () => {
      ...
      assert.equal(para.content, '<a href="http://localhost" target="_blank" rel="noopener">Text</a> on the 21<sup>st</sup> and 22<sup>nd</sup>')
    })

    test('single-line subscript chars', async () => {
      const para = await blockFromString('H~2~O')
      assert.equal(para.subQuotes(para.source), 'H<sub>2</sub>O')
    })

    test('escaped single-line subscript chars', async () => {
      const para = await blockFromString(`H${BACKSLASH}~2~O`)
      assert.equal(para.subQuotes(para.source), 'H~2~O')
    })
    ...
    test('quoted text with role shorthand', async () => {
      ...
      assert.equal(para.subQuotes(para.source), '<span class="white red-background">alert</span>')
    })

    test('quoted text with id shorthand', async () => {
      const para = await blockFromString('[#bond]#007#')
      assert.equal(para.subQuotes(para.source), '<span id="bond">007</span>')
    })

    test('quoted text with id and role shorthand', async () => {
      const para = await blockFromString('[#bond.white.red-background]#007#')
      assert.equal(para.subQuotes(para.source), '<span id="bond" class="white red-background">007</span>')
    })
    ...
    test('should ignore attributes after comma', async () => {
      ...
      assert.equal(para.subQuotes(para.source), '<span class="red">alert</span>')
    })
    ...
    test('should not assign role if value before comma is empty', async () => {
      ...
      assert.equal(para.subQuotes(para.source), 'anonymous')
    })
    ...
    test('inline passthrough with id and role set using shorthand', async () => {
      for (const attrlist of ['#idname.rolename', '.rolename#idname']) {
        ...
        assert.equal(para.content, '<span id="idname" class="rolename">pass</span>')
      }
    })
  })
})