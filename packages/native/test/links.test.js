...
describe('Links', () => {
  ...
  test('qualified url inline with text', async () => {
    const output = await convertString('The AsciiDoc project is located at http://asciidoc.org.')
    assertXpath(output, '//a[@href="http://asciidoc.org"][@class="bare"][text() = "http://asciidoc.org"]', 1)
  })

  test('qualified url with role inline with text', async () => {
    const output = await convertString('The AsciiDoc project is located at http://asciidoc.org[role=project].')
    assertXpath(output, '//a[@href="http://asciidoc.org"][@class="bare project"][text() = "http://asciidoc.org"]', 1)
  })

  test('qualified http url inline with hide-uri-scheme set', async () => {
    const output = await convertString('The AsciiDoc project is located at http://asciidoc.org.', { attributes: { 'hide-uri-scheme': '' } })
    assertXpath(output, '//a[@href="http://asciidoc.org"][@class="bare"][text() = "asciidoc.org"]', 1)
  })

  test('qualified file url inline with label', async () => {
    const output = await convertStringToEmbedded('file:///home/user/bookmarks.html[My Bookmarks]')
    assertXpath(output, '//a[@href="file:///home/user/bookmarks.html"][text() = "My Bookmarks"]', 1)
  })

  test('qualified file url inline with hide-uri-scheme set', async () => {
    const output = await convertString('Edit the configuration file link:file:///etc/app.conf[]', { attributes: { 'hide-uri-scheme': '' } })
    assertXpath(output, '//a[@href="file:///etc/app.conf"][text() = "/etc/app.conf"]', 1)
  })
  ...
  test('should not hide bare URI scheme in implicit text of link macro when hide-uri-scheme is set', async () => {
    ...
    for (const [input, expected] of cases) {
      const output = await convertInlineString(input, { attributes: { 'hide-uri-scheme': '' } })
      assertXpath(output, `/a[text() = "${expected}"]`, 1)
    }
  })

  test('qualified url with label', async () => {
    const output = await convertString("We're parsing http://asciidoc.org[AsciiDoc] markup")
    assertXpath(output, '//a[@href="http://asciidoc.org"][text() = "AsciiDoc"]', 1)
  })

  test('qualified url with label containing escaped right square bracket', async () => {
    const output = await convertString("We're parsing http://asciidoc.org[[Ascii\\]Doc] markup")
    assertXpath(output, '//a[@href="http://asciidoc.org"][text() = "[Ascii]Doc"]', 1)
  })

  test('qualified url with backslash label', async () => {
    const output = await convertString('I advise you to https://google.com[Google for +\\+]')
    assertXpath(output, '//a[@href="https://google.com"][text() = "Google for \\"]', 1)
  })

  test('qualified url with label using link macro', async () => {
    const output = await convertString("We're parsing link:http://asciidoc.org[AsciiDoc] markup")
    assertXpath(output, '//a[@href="http://asciidoc.org"][text() = "AsciiDoc"]', 1)
  })

  test('qualified url with role using link macro', async () => {
    const output = await convertString("We're parsing link:http://asciidoc.org[role=project] markup")
    assertXpath(output, '//a[@href="http://asciidoc.org"][@class="bare project"][text() = "http://asciidoc.org"]', 1)
  })

  test('qualified url using macro syntax with multi-line label inline with text', async () => {
    const output = await convertString("We're parsing link:http://asciidoc.org[AsciiDoc\nmarkup]")
    assertXpath(output, `//a[@href='http://asciidoc.org'][text() = 'AsciiDoc\nmarkup']`, 1)
  })
  ...
  test('should not recognize link macro with double colons', async () => {
    const output = await convertStringToEmbedded('The link::http://example.org[example domain] is reserved for tests and documentation.')
    assert.ok(output.includes('link::http://example.org[example domain]'))
  })

  test('qualified url surrounded by angled brackets', async () => {
    const output = await convertString('<http://asciidoc.org> is the project page for AsciiDoc.')
    assertXpath(output, '//a[@href="http://asciidoc.org"][@class="bare"][text()="http://asciidoc.org"]', 1)
  })
  ...
  test('qualified url surrounded by double angled brackets should preserve outer angled brackets', async () => {
    ...
    assert.ok(output.includes('&lt;<a href="https://asciidoc.org" class="bare">https://asciidoc.org</a>&gt;'))
  })
  ...
  test('multiple qualified urls surrounded by angled brackets in unconstrained context', async () => {
    const output = await convertString('URLは<http://asciidoc.org>。URLは<http://asciidoc.org>。')
    assertXpath(output, '//a[@href="http://asciidoc.org"][@class="bare"][text()="http://asciidoc.org"]', 2)
  })
  ...
  test('xref shorthand with target that starts with URL protocol and has space after comma should not crash parser', async () => {
    const output = await convertStringToEmbedded('<<https://example.com, Example>>')
  ...
  test('xref shorthand with link macro as target should be ignored', async () => {
    const output = await convertStringToEmbedded('<<link:https://example.com[], Example>>')
    assert.ok(output.includes('&lt;&lt;<a href="https://example.com" class="bare">https://example.com</a>, Example&gt;&gt;'))
  })
  ...
  test('autolink containing text enclosed in angle brackets', async () => {
    ...
    assert.ok(output.includes('<a href="https://github.com/&lt;org&gt;/" class="bare">https://github.com/&lt;org&gt;/</a>'))
  })

  test('qualified url surrounded by round brackets', async () => {
    const output = await convertString('(http://asciidoc.org) is the project page for AsciiDoc.')
    assertXpath(output, '//a[@href="http://asciidoc.org"][text()="http://asciidoc.org"]', 1)
  })

  test('qualified url with trailing period', async () => {
    const result = await convertStringToEmbedded('The homepage for Asciidoctor is https://asciidoctor.org.')
    assertXpath(result, '//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]', 1)
    assertXpath(result, '//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]/following-sibling::text()[starts-with(.,".")]', 1)
  })

  test('qualified url with trailing exclamation point', async () => {
    const result = await convertStringToEmbedded('Check out https://asciidoctor.org!')
    assertXpath(result, '//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]', 1)
    assertXpath(result, '//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]/following-sibling::text()[starts-with(.,"!")]', 1)
  })

  test('qualified url with trailing question mark', async () => {
    const result = await convertStringToEmbedded('Is the homepage for Asciidoctor https://asciidoctor.org?')
    assertXpath(result, '//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]', 1)
    assertXpath(result, '//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]/following-sibling::text()[starts-with(.,"?")]', 1)
  })

  test('qualified url with trailing round bracket', async () => {
    const result = await convertStringToEmbedded('Asciidoctor is a Ruby-based AsciiDoc processor (see https://asciidoctor.org)')
    assertXpath(result, '//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]', 1)
    assertXpath(result, '//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]/following-sibling::text()[starts-with(.,")")]', 1)
  })
  ...
  test('qualified url with trailing semi-colon', async () => {
    const result = await convertStringToEmbedded('https://asciidoctor.org; where text gets parsed')
    assertXpath(result, '//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]', 1)
    assertXpath(result, '//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]/following-sibling::text()[starts-with(.,";")]', 1)
  })
  ...
  test('qualified url with trailing round bracket followed by colon', async () => {
    const result = await convertStringToEmbedded('(from https://asciidoctor.org): where text gets parsed')
    assertXpath(result, '//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]', 1)
    assertXpath(result, '//a[@href="https://asciidoctor.org"][text()="https://asciidoctor.org"]/following-sibling::text()[starts-with(., "):")]', 1)
  })
  ...
  test('qualified url containing round brackets', async () => {
    const output = await convertString('http://jruby.org/apidocs/org/jruby/Ruby.html#addModule(org.jruby.RubyModule)[addModule() adds a Ruby module]')
    assertXpath(output, '//a[@href="http://jruby.org/apidocs/org/jruby/Ruby.html#addModule(org.jruby.RubyModule)"][text()="addModule() adds a Ruby module"]', 1)
  })

  test('qualified url adjacent to text in square brackets', async () => {
    const output = await convertString(']http://asciidoc.org[AsciiDoc] project page.')
    assertXpath(output, '//a[@href="http://asciidoc.org"][text()="AsciiDoc"]', 1)
  })
  ...
  test('qualified url following no-break space', async () => {
    const output = await convertString(`${String.fromCodePoint(0xa0)}http://asciidoc.org[AsciiDoc] project page.`)
    assertXpath(output, '//a[@href="http://asciidoc.org"][text()="AsciiDoc"]', 1)
  })

  test('qualified url following smart apostrophe', async () => {
    const output = await convertStringToEmbedded('l&#8217;http://www.irit.fr[IRIT]')
  ...
  test('should convert qualified url as macro enclosed in double quotes', async () => {
    ...
    assert.ok(output.includes('"<a href="https://asciidoctor.org" class="bare">https://asciidoctor.org</a>"'))
  })
  ...
  test('should convert qualified url as macro with trailing period', async () => {
    const result = await convertStringToEmbedded('Information about the https://symbols.example.org/.[.] character.')
    assertXpath(result, '//a[@href="https://symbols.example.org/."][text()="."]', 1)
  })

  test('qualified url using invalid link macro should not create link', async () => {
    const output = await convertString('link:http://asciidoc.org is the project page for AsciiDoc.')
  ...
  test('escaped inline qualified url as macro should not create link', async () => {
    const output = await convertString('\\http://asciidoc.org[asciidoc.org] is the project page for AsciiDoc.')
    assertXpath(output, '//a', 0)
    assertXpath(output, '//p[starts-with(text(), "http://asciidoc.org[asciidoc.org]")]', 1)
  })

  test('url in link macro with at (@) sign should not create mailto link', async () => {
    const output = await convertString('http://xircles.codehaus.org/lists/dev@geb.codehaus.org[subscribe]')
    assertXpath(output, '//a[@href="http://xircles.codehaus.org/lists/dev@geb.codehaus.org"][text()="subscribe"]', 1)
  })

  test('implicit url with at (@) sign should not create mailto link', async () => {
    const output = await convertString('http://xircles.codehaus.org/lists/dev@geb.codehaus.org')
    assertXpath(output, '//a[@href="http://xircles.codehaus.org/lists/dev@geb.codehaus.org"][text()="http://xircles.codehaus.org/lists/dev@geb.codehaus.org"]', 1)
  })

  test('escaped inline qualified url using macro syntax should not create link', async () => {
    const output = await convertString('\\http://asciidoc.org[AsciiDoc] is the key to good docs.')
    assertXpath(output, '//a', 0)
  })

  test('inline qualified url followed by a newline should not include newline in link', async () => {
    const output = await convertString("The source code for Asciidoctor can be found at https://github.com/asciidoctor\nwhich is a GitHub organization.")
  ...
  test('qualified url divided by newline using macro syntax should not create link', async () => {
    const output = await convertString("The source code for Asciidoctor can be found at link:https://github.com/asciidoctor\n[]which is a GitHub organization.")
    assertXpath(output, '//a', 0)
  })

  test('qualified url containing whitespace using macro syntax should not create link', async () => {
    const output = await convertString('I often need to refer to the chapter on link:http://asciidoc.org?q=attribute references[Attribute References].')
    assertXpath(output, '//a', 0)
  })

  test('qualified url containing an encoded space using macro syntax should create a link', async () => {
    const output = await convertString('I often need to refer to the chapter on link:http://asciidoc.org?q=attribute%20references[Attribute References].')
    assertXpath(output, '//a', 1)
  })

  test('inline quoted qualified url should not consume surrounding angled brackets', async () => {
    const output = await convertString('Asciidoctor GitHub organization: <**https://github.com/asciidoctor**>')
    assertXpath(output, '//a[@href="https://github.com/asciidoctor"]', 1)
  })

  test('link with quoted text should not be separated into attributes when text contains an equal sign', async () => {
    const output = await convertStringToEmbedded('http://search.example.com["Google, Yahoo, Bing = Search Engines"]')
    assertXpath(output, '//a[@href="http://search.example.com"][text()="Google, Yahoo, Bing = Search Engines"]', 1)
  })

  test('should leave link text as is if it contains an equals sign but no attributes are found', async () => {
    const output = await convertStringToEmbedded('https://example.com[What You Need\n= What You Get]')
    assertXpath(output, '//a[@href="https://example.com"][text()="What You Need\n= What You Get"]', 1)
  })

  test('link with quoted text but no equal sign should carry quotes over to output', async () => {
    const output = await convertStringToEmbedded('http://search.example.com["Google, Yahoo, Bing"]')
    assertXpath(output, '//a[@href="http://search.example.com"][text()=\'"Google, Yahoo, Bing"\']', 1)
  })
  ...
  test('link with formatted wrapped text should not be separated into attributes', async () => {
    const result = await convertStringToEmbedded('https://example.com[[.role]#Foo\nBar#]')
    assert.ok(result.includes('<a href="https://example.com"><span class="role">Foo\nBar</span></a>'))
  })

  test('should process role and window attributes on link', async () => {
    const output = await convertStringToEmbedded('http://google.com[Google, role=external, window="_blank"]')
    assertXpath(output, '//a[@href="http://google.com"][@class="external"][@target="_blank"]', 1)
  })

  test('should parse link with wrapped text that includes attributes', async () => {
    const result = await convertStringToEmbedded('https://example.com[Foo\nBar,role=foobar]')
    assert.ok(result.includes('<a href="https://example.com" class="foobar">Foo Bar</a>'))
  })
  ...
  test('link macro with attributes but no text should use URL as text', async () => {
    ...
    const output = await convertStringToEmbedded(`link:${url}[family=Roboto,weight=400]`)
  ...
  test('link macro should support id and role attributes', async () => {
    ...
    const output = await convertStringToEmbedded(`link:${url}[,id=roboto-regular,role=font]`)
    assertXpath(output, `//a[@href="${url}"][@id="roboto-regular"][@class="bare font"][text()="${url}"]`, 1)
  })
  ...
  test('rel=noopener should be added to a link that targets the _blank window', async () => {
    const output = await convertStringToEmbedded('http://google.com[Google^]')
    assertXpath(output, '//a[@href="http://google.com"][@target="_blank"][@rel="noopener"]', 1)
  })

  test('rel=noopener should be added to a link that targets a named window when the noopener option is set', async () => {
    const output = await convertStringToEmbedded('http://google.com[Google,window=name,opts=noopener]')
    assertXpath(output, '//a[@href="http://google.com"][@target="name"][@rel="noopener"]', 1)
  })

  test('rel=noopener should not be added to a link if it does not target a window', async () => {
    const result = await convertStringToEmbedded('http://google.com[Google,opts=noopener]')
  ...
  test('rel=nofollow should be added to a link when the nofollow option is set', async () => {
    const output = await convertStringToEmbedded('http://google.com[Google,window=name,opts="nofollow,noopener"]')
    assertXpath(output, '//a[@href="http://google.com"][@target="name"][@rel="nofollow noopener"]', 1)
  })

  test('id attribute on link is processed', async () => {
    const output = await convertStringToEmbedded('http://google.com[Google, id="link-1"]')
    assertXpath(output, '//a[@href="http://google.com"][@id="link-1"]', 1)
  })

  test('title attribute on link is processed', async () => {
    const output = await convertStringToEmbedded('http://google.com[Google, title="title-1"]')
  ...
  test('inline irc link', async () => {
    ...
    assertXpath(output, '//a[@href="irc://irc.freenode.net"][text()="irc://irc.freenode.net"]', 1)
  })

  test('inline irc link with text', async () => {
    const output = await convertStringToEmbedded('irc://irc.freenode.net[Freenode IRC]')
    assertXpath(output, '//a[@href="irc://irc.freenode.net"][text()="Freenode IRC"]', 1)
  })
  ...
  test('should encode double quotes in reftext of anchor macro in DocBook output', async () => {
    ...
    assert.equal(result, '<anchor xml:id="uncola" xreflabel="the &quot;un&quot;-cola"/>')
  })
  ...
  test('does not match bibliography anchor in prose when scanning for inline anchor', async () => {
    const doc = await documentFromString('Use [[[label]]] to assign a label to a bibliography entry, but not in a paragraph.')
  ...
  test('mixed inline anchor macro and anchor shorthand with empty reftext', async () => {
    ...
    const result = await convertInlineString(input)
    assert.equal(result, '<a id="one"></a><a id="two"></a><a id="three"></a><a id="four"></a><a id="five"></a>')
  })
  ...
  test('xref using angled bracket syntax with label', async () => {
    ...
    assertXpath(await convertString(input), '//a[@href="#tigers"][text() = "About Tigers"]', 1)
  })

  test('xref should use title of target as link text when no explicit reftext is specified', async () => {
    const input = '<<tigers>>\n\n[#tigers]\n== Tigers'
    assertXpath(await convertString(input), '//a[@href="#tigers"][text() = "Tigers"]', 1)
  })
  ...
  test('inter-document xref shorthand syntax should assume AsciiDoc extension if AsciiDoc extension not present', async () => {
    ...
    for (const [target, text] of cases) {
      ...
      assertXpath(result, `//a[@href="${target.slice(0, -1)}.html"][text()="${text}"]`, 1)
    }
  })
  ...
  test('xref macro with explicit inter-document target should assume implicit AsciiDoc file extension if no file extension is present', async () => {
    ...
    for (const [target, text] of cases1) {
      ...
      assertXpath(result, `//a[@href="${target.slice(0, -1)}"][text()="${text}"]`, 1)
    }
    const cases2 = [
  ...
  test('inter-document xref should only remove the file extension part if the path contains a period elsewhere', async () => {
    const result = await convertStringToEmbedded('<<using-.net-web-services.adoc#,Using .NET web services>>')
    assertXpath(result, '//a[@href="using-.net-web-services.html"][text() = "Using .NET web services"]', 1)
  })

  test('xref macro target containing dot should be interpreted as a path unless prefixed by #', async () => {
    let result = await convertStringToEmbedded('xref:using-.net-web-services[Using .NET web services]')
    assertXpath(result, '//a[@href="using-.net-web-services"][text() = "Using .NET web services"]', 1)
    result = await convertStringToEmbedded('xref:#using-.net-web-services[Using .NET web services]')
    assertXpath(result, '//a[@href="#using-.net-web-services"][text() = "Using .NET web services"]', 1)
  })

  test('should not interpret double underscore in target of xref macro if sequence is preceded by a backslash', async () => {
    const result = await convertStringToEmbedded('xref:doc\\__with_double__underscore.adoc[text]')
    assertXpath(result, '//a[@href="doc__with_double__underscore.html"][text() = "text"]', 1)
  })

  test('should not interpret double underscore in target of xref shorthand if sequence is preceded by a backslash', async () => {
    const result = await convertStringToEmbedded('<<doc\\__with_double__underscore.adoc#,text>>')
    assertXpath(result, '//a[@href="doc__with_double__underscore.html"][text() = "text"]', 1)
  })
  ...
  test('xref using angled bracket syntax inline with text', async () => {
    const input = 'Want to learn <<tigers,about tigers>>?\n\n[#tigers]\n== Tigers'
    assertXpath(await convertString(input), '//a[@href="#tigers"][text() = "about tigers"]', 1)
  })

  test('xref using angled bracket syntax with multi-line label inline with text', async () => {
    const input = 'Want to learn <<tigers,about\ntigers>>?\n\n[#tigers]\n== Tigers'
    assertXpath(await convertString(input), `//a[@href="#tigers"][normalize-space(text()) = "about tigers"]`, 1)
  })

  test('xref with escaped text', async () => {
    const input = 'See the <<tigers, `+[tigers]+`>> section for details about tigers.\n\n[#tigers]\n== Tigers'
  ...
  test('xref with target that begins with attribute reference in title', async () => {
    for (const xref of ['<<{lessonsdir}/lesson-1#,Lesson 1>>', 'xref:{lessonsdir}/lesson-1.adoc[Lesson 1]']) {
  ...
  test('multiple xref macros with implicit text in single line', async () => {
    const input = 'This document has two sections, xref:sect-a[] and xref:sect-b[].\n\n[#sect-a]\n== Section A\n\n[#sect-b]\n== Section B'
    const result = await convertStringToEmbedded(input)
  ...
  test('xref using macro syntax with text that contains an escaped closing bracket', async () => {
    ...
    assertXpath(await convertStringToEmbedded(input), '//a[@href="#tigers"][text() = "[tigers] are cats"]', 1)
  })
  ...
  test('should warn and create link if inter-document xref points to current doc and reference not found', async () => {
    ...
    const output = await convertStringToEmbedded(input, { attributes: { docname: 'test' } })
    assertXpath(output, '//a[@href="#foobaz"][text() = "[foobaz]"]', 1)
    assertMessage(logger, 'info', 'possible invalid reference: foobaz')
  })
  ...
  test('should use doctitle as fallback link text if inter-document xref points to current doc and no link text is provided', async () => {
    const input = '= Links & Stuff at https://example.org\n\nSee xref:test.adoc[]'
    const output = await convertStringToEmbedded(input, { attributes: { docname: 'test' } })
    assert.ok(output.includes('<a href="#">Links &amp; Stuff at https://example.org</a>'))
  })
  ...
  test('should use document id as linkend for self xref in DocBook backend', async () => {
    const input = '[#docid]\n= Document Title\n\nSee xref:test.adoc[]'
    const output = await convertStringToEmbedded(input, { backend: 'docbook', attributes: { docname: 'test' } })
  ...
  test('should not fail to resolve broken xref in title of block with ID', async () => {
    const input = '[#p1]\n.<<DNE>>\nparagraph text'
    const output = await convertStringToEmbedded(input)
    assertXpath(output, '//*[@class="title"]/a[@href="#DNE"][text()="[DNE]"]', 1)
  })

  test('should resolve forward xref in title of block with ID', async () => {
    const input = '[#p1]\n.<<conclusion>>\nparagraph text\n\n[#conclusion]\n== Conclusion'
    const output = await convertStringToEmbedded(input)
    assertXpath(output, '//*[@class="title"]/a[@href="#conclusion"][text()="Conclusion"]', 1)
  })
  ...
  test('should drop nested anchor in xreftext', async () => {
    ...
    const output = await convertStringToEmbedded(input)
    assert.ok(output.includes('<h2 id="a">See <a href="#b">Consult Google</a></h2>'))
  })
  ...
  test('should not match numeric character references while searching for fragment in xref target', async () => {
    ...
    assertXpath(output, `//a[@href="#_cub_tiger"][text()="Cub ${decodeChar(8658)} Tiger"]`, 1)
  })
  ...
  test('anchor with quoted label creates reference with quoted label text', async () => {
    const doc = await documentFromString('[[tigers,"Tigers roam here"]]Tigers roam here.')
    const ref = doc.catalog.refs['tigers']
    assert.ok(ref != null)
  ...
  test('anchor with label containing a comma creates reference', async () => {
    const doc = await documentFromString('[[tigers,Tigers, scary tigers, roam here]]Tigers roam here.')
    const ref = doc.catalog.refs['tigers']
    assert.ok(ref != null)
