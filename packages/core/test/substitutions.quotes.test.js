...
describe('Substitutions', () => {
  describe('Quotes', () => {
    test('single-line double-quoted string', async () => {
      let para = await blockFromString("``a few quoted words''", {
        attributes: { 'compat-mode': '' },
      })
      assert.equal(
        await para.subQuotes(para.source),
        '&#8220;a few quoted words&#8221;'
      )
    ...
    test('escaped single-line double-quoted string', async () => {
      let para = await blockFromString(`${BACKSLASH}\`\`a few quoted words''`, {
        attributes: { 'compat-mode': '' },
      })
      assert.equal(
        await para.subQuotes(para.source),
      ...
      para = await blockFromString(
        `${BACKSLASH.repeat(2)}\`\`a few quoted words''`,
        { attributes: { 'compat-mode': '' } }
      )
    ...
    test('single-quoted string with inline single quote', async () => {
      let para = await blockFromString("`That isn't what I did.'", {
        attributes: { 'compat-mode': '' },
      })
      assert.equal(
        await para.subQuotes(para.source),
    ...
    test('escaped single-line unconstrained marked string', async () => {
      const para = await blockFromString(
        `${BACKSLASH.repeat(2)}##--anything goes ##`
      )
    ...
    test('should ignore enclosing square brackets when processing formatted text with attribute list', async () => {
      const doc = await documentFromString('nums = [1, 2, 3, [.blue]#4#]', {
        doctype: 'inline',
      })
      assert.equal(
        await doc.convert(),
    ...
    test('escaped single quoted string', async () => {
      ...
      assert.equal(
        await para.subQuotes(para.source),
        `${BACKSLASH}'a few emphasized words'`
      )
    })
    ...
    test('escaped single-quotes inside emphasized words are restored', async () => {
      let para = await blockFromString(`'Here${BACKSLASH}'s Johnny!'`, {
        attributes: { 'compat-mode': '' },
      })
    ...
    test('single-line constrained monospaced string', async () => {
      let para = await blockFromString('`a few <{monospaced}> words`', {
      ...
      para = await blockFromString('`a few <{monospaced}> words`', {
    ...
    test('single-line constrained monospaced string with role', async () => {
      let para = await blockFromString('[input]`a few <{monospaced}> words`', {
        attributes: { monospaced: 'monospaced', 'compat-mode': '' },
      })
      assert.equal(
        await para.applySubs(para.source),
      ...
      para = await blockFromString('[input]`a few <{monospaced}> words`', {
        attributes: { monospaced: 'monospaced' },
      })
      assert.equal(
        await para.applySubs(para.source),
    ...
    test('escaped single-line constrained monospaced string', async () => {
      let para = await blockFromString(
        `${BACKSLASH}\`a few <monospaced> words\``,
        { attributes: { 'compat-mode': '' } }
      )
      assert.equal(
        await para.applySubs(para.source),
    ...
    test('escaped single-line constrained monospaced string with role', async () => {
      let para = await blockFromString(
        `[input]${BACKSLASH}\`a few <monospaced> words\``,
        { attributes: { 'compat-mode': '' } }
      )
      assert.equal(
        await para.applySubs(para.source),
    ...
    test('escaped role on single-line constrained monospaced string', async () => {
      let para = await blockFromString(
        `${BACKSLASH}[input]\`a few <monospaced> words\``,
        { attributes: { 'compat-mode': '' } }
      )
      assert.equal(
        await para.applySubs(para.source),
    ...
    test('escaped role on escaped single-line constrained monospaced string', async () => {
      let para = await blockFromString(
        `${BACKSLASH}[input]${BACKSLASH}\`a few <monospaced> words\``,
        { attributes: { 'compat-mode': '' } }
      )
      assert.equal(
        await para.applySubs(para.source),
    ...
    test('single-line constrained monospaced chars', async () => {
      let para = await blockFromString('call +save()+ to persist the changes', {
        attributes: { 'compat-mode': '' },
      })
      assert.equal(
        await para.subQuotes(para.source),
    ...
    test('escaped single-line constrained monospaced chars', async () => {
      let para = await blockFromString(
        `call ${BACKSLASH}+save()+ to persist the changes`,
        { attributes: { 'compat-mode': '' } }
      )
      assert.equal(
        await para.subQuotes(para.source),
    ...
    test('escaped single-line constrained monospaced chars with role', async () => {
      let para = await blockFromString(
        `call [method]${BACKSLASH}+save()+ to persist the changes`,
        { attributes: { 'compat-mode': '' } }
      )
      assert.equal(
        await para.subQuotes(para.source),
    ...
    test('escaped role on single-line constrained monospaced chars', async () => {
      let para = await blockFromString(
        `call ${BACKSLASH}[method]+save()+ to persist the changes`,
        { attributes: { 'compat-mode': '' } }
      )
      assert.equal(
        await para.subQuotes(para.source),
    ...
    test('escaped role on escaped single-line constrained monospaced chars', async () => {
      let para = await blockFromString(
        `call ${BACKSLASH}[method]${BACKSLASH}+save()+ to persist the changes`,
        { attributes: { 'compat-mode': '' } }
      )
      assert.equal(
        await para.subQuotes(para.source),
    ...
    test('single-line unconstrained monospaced chars', async () => {
      let para = await blockFromString('Git++Hub++', {
        attributes: { 'compat-mode': '' },
      })
    ...
    test('escaped single-line unconstrained monospaced chars', async () => {
      let para = await blockFromString(`Git${BACKSLASH}++Hub++`, {
        attributes: { 'compat-mode': '' },
      })
      assert.equal(await para.subQuotes(para.source), 'Git+<code>Hub</code>+')

      para = await blockFromString(`Git${BACKSLASH.repeat(2)}++Hub++`, {
        attributes: { 'compat-mode': '' },
      })
    ...
    test('should allow role to be defined using attribute reference', async () => {
      const doc = await documentFromString('[{rolename}]#phrase#', {
        doctype: 'inline',
