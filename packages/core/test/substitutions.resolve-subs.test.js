...
describe('Substitutions', () => {
  describe('Resolve subs', () => {
    test('should resolve subs for block', async () => {
      ...
      block.commitSubs()
      assert.deepEqual(block.subs, ['quotes', 'specialcharacters', 'attributes', 'replacements', 'macros', 'post_replacements'])
    })
    ...
    test('should not use subs if subs option passed to block constructor is null', async () => {
      const doc = await emptyDocument()
      const block = new Block(doc, 'paragraph', { source: '*bold* _italic_', subs: null, attributes: { subs: 'quotes' } })
    ...
    test('should not use subs if subs option passed to block constructor is empty array', async () => {
      const doc = await emptyDocument()
      const block = new Block(doc, 'paragraph', { source: '*bold* _italic_', subs: [], attributes: { subs: 'quotes' } })
      assert.deepEqual(block.subs, [])
      block.commitSubs()
      assert.deepEqual(block.subs, [])
    })
    ...
    test('should use subs from subs option passed to block constructor', async () => {
      const doc = await emptyDocument()
      const block = new Block(doc, 'paragraph', { source: '*bold* _italic_', subs: ['specialcharacters'], attributes: { subs: 'quotes' } })
      assert.deepEqual(block.subs, ['specialcharacters'])
      block.commitSubs()
      assert.deepEqual(block.subs, ['specialcharacters'])
    })
    ...
    test('should use subs from subs attribute if subs option is not passed to block constructor', async () => {
      const doc = await emptyDocument()
      const block = new Block(doc, 'paragraph', { source: '*bold* _italic_', attributes: { subs: 'quotes' } })
      assert.deepEqual(block.subs, [])
    ...
    test('should use subs from subs attribute if subs option passed to block constructor is :default', async () => {
      const doc = await emptyDocument()
      const block = new Block(doc, 'paragraph', { source: '*bold* _italic_', subs: 'default', attributes: { subs: 'quotes' } })
      assert.deepEqual(block.subs, ['quotes'])
      block.commitSubs()
      assert.deepEqual(block.subs, ['quotes'])
    })
    ...
    test('should use built-in subs if subs option passed to block constructor is :default and subs attribute is absent', async () => {
      const doc = await emptyDocument()
      const block = new Block(doc, 'paragraph', { source: '*bold* _italic_', subs: 'default' })
      assert.deepEqual(block.subs, ['specialcharacters', 'quotes', 'attributes', 'replacements', 'macros', 'post_replacements'])
      block.commitSubs()
      assert.deepEqual(block.subs, ['specialcharacters', 'quotes', 'attributes', 'replacements', 'macros', 'post_replacements'])
    })
  })
