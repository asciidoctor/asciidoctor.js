...
describe('Using a template converter', () => {
  test('should use a Pug template', async () => {
    ...
    assert.ok(
      result.includes('<p class="paragraph-pug">content</p>'),
      `Unexpected output:\n${result}`
    )
  })
  ...
  test('should use a Nunjucks template', async () => {
    ...
    assert.ok(
      result.includes('<p class="paragraph-nunjucks">content</p>'),
      `Unexpected output:\n${result}`
    )
  })
  ...
  test('should use an EJS template', async () => {
    const result = await convert('content', {
      safe: 'safe',
      backend: 'html5',
      template_dir: join(TEMPLATES_DIR, 'ejs'),
    })
    assert.ok(
      result.includes('<p class="paragraph-ejs">content</p>'),
      `Unexpected output:\n${result}`
    )
  })
  ...
  test('should use a Handlebars template', async () => {
    ...
    assert.ok(
      result.includes('<p class="paragraph-handlebars">content</p>'),
      `Unexpected output:\n${result}`
    )
  })
  ...
  test('should use a doT template', async () => {
    const options = {
      safe: 'safe',
      backend: 'html5',
      template_dir: join(TEMPLATES_DIR, 'dot'),
      template_engine: 'dot',
    }
    ...
    assert.ok(
      result.includes('<p class="paragraph-dot">content</p>'),
      `Unexpected output:\n${result}`
    )
  })

  test('should use a JavaScript template', async () => {
    const result = await convert('*bold* statement', {
      safe: 'safe',
      backend: 'html5',
      template_dir: join(TEMPLATES_DIR, 'js'),
    })
    assert.ok(
      result.includes(
        '<p class="paragraph-js"><strong>bold</strong> statement</p>'
      ),
      `Unexpected output:\n${result}`
    )
  })
  ...
  test('should convert a given node', async () => {
    ...
    assert.equal(
      result,
      '<p class="paragraph-nunjucks">This is a &lt;test&gt;</p>'
    )
  })
  ...
  test('should get the template converter caches', async () => {
    ...
    assert.ok(
      cache.scans[templatesPattern].paragraph,
      'Expected paragraph in scan cache'
    )
    const templateFilePath = resolve(templateDir, 'paragraph.njk')
    assert.ok(
      cache.templates[templateFilePath],
      'Expected template file in template cache'
    )
  })
  ...
  <div class="message-body">
    ...
    const templates = templateConverter.getTemplates()
    assert.ok(
      templates.admonition,
      'Expected admonition template to be registered'
    )
  })
...
`,
      {
        ...
        backend: 'html5',
        template_dirs: [
          join(TEMPLATES_DIR, 'nunjucks-ctx-a'),
          join(TEMPLATES_DIR, 'nunjucks-ctx-b'),
        ],
      }
    )
    assert.equal(
      result,
      `<p class="paragraph"><img class="inline" src="https://cdn.jsdelivr.net/a.png"/></p>
<img src="https://cdn.statically.io/b.png"/>`
  ...
  test('should allow to use different template engines inside the same directory', async () => {
    const result = await convert(
      `This a paragraph with an inline image image:b.png[].
...
`,
      {
        safe: 'safe',
        backend: 'html5',
        template_dir: join(TEMPLATES_DIR, 'mixed'),
      }
    )
    assert.equal(
      result,
      `<p class="paragraph-handlebars">This a paragraph with an inline image <img class="inline" src="https://cdn.jsdelivr.net/b.png"/>.
...
`,
      {
        safe: 'safe',
        backend: 'html5',
        template_dir: join(TEMPLATES_DIR, 'nunjucks-include'),
      }
    )
    assert.equal(
      result.replace(/\r/g, '').replace(/\n/g, ''),
      '<ul class="ulist"><p>foo</p><p>bar</p><p>baz</p></ul>'
    )
  })

  test('should configure Nunjucks environment using the template_engine_options', async () => {
    const result = await convert(
      'Simple paragraph with an inline image image:cat.png[]',
      {
        safe: 'safe',
        backend: 'html5',
        template_dir: join(TEMPLATES_DIR, 'nunjucks'),
    ...
    assert.equal(
      result,
      '<p class="paragraph-nunjucks">Simple paragraph with an inline image <span class="image"><img src="cat.png" alt="cat"></span></p>'
    )
  })
  ...
  test('should configure Handlebars environment using the template_engine_options', async () => {
    ...
    assert.equal(
      result,
      '<p class="paragraph-handlebars">Simple paragraph with an inline image <span class="image"><img src="cat.png" alt="cat"></span></p>'
    )
  })
  ...
  test('should configure Pug templates using the template_engine_options', async () => {
    ...
    assert.equal(
      result,
      '<p class="paragraph-pug"><img src="cat.png"></img></p>'
    )
  })
...
content`
      const result = (
        await convert(content, {
          safe: 'safe',
          backend: 'html5',
          template_dir: join(TEMPLATES_DIR, 'handlebars-json'),
        })
      ).replace(/\s{2,}|\r?\n|\r/g, '')
      const expected =
        '<div id="id1" class="paragraph-handlebars"><p>content</p>\n</div>'.replace(
          /\s{2,}|\r?\n|\r/g,
          ''
        )
      assert.ok(
