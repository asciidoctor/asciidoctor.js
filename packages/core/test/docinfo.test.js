...
const defs = {
  ...
  'docinfo=private-head,shared-footer': {
    head_script: true,
    meta: false,
    top_link: true,
    footer_script: false,
  ...
  for (const [key, markup] of Object.entries(defs)) {
    test(`should include docinfo files for html backend with attribute ${key}`, async () => {
      ...
      if (markup.head_script) {
        assert.ok(
          html.includes('<script src="modernizr.js"></script>'),
          `Expected head script in:\n${html}`
        )
      } else {
        assert.ok(
          !html.includes('<script src="modernizr.js"></script>'),
          `Did not expect head script in:\n${html}`
        )
      }

      if (markup.meta) {
        assert.ok(
          html.includes('<meta http-equiv="imagetoolbar" content="false">'),
          `Expected meta in:\n${html}`
        )
      } else {
        assert.ok(
          !html.includes('<meta http-equiv="imagetoolbar" content="false">'),
          `Did not expect meta in:\n${html}`
        )
      }

      if (markup.top_link) {
        assert.ok(
          html.includes('<a id="top" href="#">Back to top</a>'),
          `Expected top link in:\n${html}`
        )
      } else {
        assert.ok(
          !html.includes('<a id="top" href="#">Back to top</a>'),
          `Did not expect top link in:\n${html}`
        )
      }
      ...
      if (markup.navbar) {
        assert.ok(
          html.includes('<nav class="navbar">'),
          `Expected navbar in:\n${html}`
        )
        assert.ok(
          html.includes('</nav>\n<div id="header">'),
          `Expected navbar closing in:\n${html}`
        )
      } else {
        assert.ok(
          !html.includes('<nav class="navbar">'),
          `Did not expect navbar in:\n${html}`
        )
      }
    })
  }
