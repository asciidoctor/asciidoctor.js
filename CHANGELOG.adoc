...
* Add an experimental `semantic-html5` backend (`convert(input, { backend: 'semantic-html5' })`) that maps every AsciiDoc node to the HTML element that carries its meaning -- `<section>`, `<figure>`/`<figcaption>`, `<aside>`, `<blockquote>`, `<details>`, `<nav>`, `<time>`... -- instead of the AsciiDoc.py-compatible wrapper ``<div>``s of the default `html5` backend, with ARIA roles, a `<header>`/`<main>`/`<footer>` page skeleton, an optional XML syntax (`htmlsyntax=xml`) and its own default stylesheet (`asciidoctor-semantic.css`, with automatic light/dark themes). *This backend is experimental*: it is the testbed for the modern HTML output effort started upstream (asciidoctor/asciidoctor#242), so its markup, class names and stylesheet may change in any release, and it does not replace the `html5` backend. The converter class is exported as `SemanticHtml5Converter`. See https://docs.asciidoctor.org/asciidoctor.js/latest/whats-new/[What's New] for examples, and `devdocs/semantic-html5-converter.adoc` for the element mapping and design choices
* Add an experimental build-time mode to the built-in `highlightjs` source highlighter: set `:highlightjs-mode: build` to colorize source blocks at conversion time instead of in the browser. It supports callouts, `linenums`, `highlight=` and `start=`, and embeds the theme stylesheet (`:highlightjs-theme:`, default `github`; `:highlightjs-stylesheet: link` to link the CDN instead). `highlight.js` is an optional `peerDependency`, loaded lazily; in the browser, the adapter falls back to client-side highlighting with a warning
* Syntax highlighter adapters may now be asynchronous: the core awaits `highlight()` and `docinfo()`, so an adapter can use an async engine such as Shiki. Existing synchronous adapters are unaffected
