...
* Add a built-in *semantic HTML5* converter registered for the `semantic-html5` backend (`Asciidoctor.convert(input, { backend: 'semantic-html5' })`). Unlike the default `html5` backend — whose output mirrors AsciiDoc.py for compatibility (wrapper `<div>`s, `class="paragraph"`, table-based admonitions, …) — this converter maximizes the use of semantic HTML constructs: `<section>` for sections, bare `<p>` for paragraphs, `<figure>`/`<figcaption>` for captioned listings and images, `<header>` with a `byline` and a revision `<table>` for the document header, `<time datetime>` for the revision date, and `rel="author"` on author email links. It ports and continues the work started in the upstream Ruby `feature/html-converter-next` branch, along with its scenario-based test suite (`.adoc`/`.html` fixture pairs in `test/fixtures/semantic-html5-scenarios/`)
