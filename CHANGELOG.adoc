...
* Wire up the `pipe` constructor option on `Logger` (`new Logger({ pipe })`), mirroring Ruby's `Logger.new(logdev)`. It previously had no effect — `_writeln()` always wrote to `process.stderr`/`console.error` regardless of the option. `pipe` now accepts anything with a `write(line)` method or a plain function, letting a custom logger redirect its formatted output without subclassing `Logger` or overriding the internal `_writeln()` method
