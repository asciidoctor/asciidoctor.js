...
* Add an experimental build-time mode to the built-in `highlightjs` source highlighter, based on https://highlightjs.org[highlight.js]. Setting `:highlightjs-mode: build` colourises source blocks at conversion time in Node instead of shipping the hljs runtime for the browser to colourise; without the attribute the behaviour is unchanged (client-side), so no registration or extra setup is required. `highlight.js` is declared as an *optional* `peerDependency` and is loaded lazily via a runtime require on first use, so importing the core never pulls it in and it is never bundled (including in the browser build); a clear error is thrown if build mode is requested on a server runtime while the package is missing. The Node-only build code lives in a separate module (`highlightjs-build.js`) that the browser build replaces with a stub, so build mode is reported as unsupported in the browser: the adapter then logs a warning and falls back to client-side highlighting rather than failing. Callouts are preserved: the core strips the conums before highlighting and re-appends them afterwards, and the adapter closes/reopens hljs spans at each line boundary so a conum is never swallowed by a multiline comment or string span. Line numbering (`linenums`) is supported in both `table` mode (returning a `[html, offset]` tuple so the core realigns the conums) and `inline` mode, along with `highlight=` line emphasis and `start=`. In build mode the theme stylesheet is read from the installed highlight.js package and embedded in a `<style>` tag by default (self-contained output); set `:highlightjs-stylesheet: link` to link the CDN instead, and `:highlightjs-theme:` to pick a theme (default `github`). A small helper stylesheet is also injected to style the build-mode markup that highlight.js themes do not cover — the inline and table line-number gutters and the emphasised-line highlight (full-width, with any callouts kept on the line)
