...
```ruby
require 'asciidoctor/extensions' unless RUBY_ENGINE == 'opal' # <1>
...
```
<1> Conditional `require` because the compiler can not resolve this dependency but Asciidoctor.js will provide it at runtime
...
const html = asciidoctor.convert(fs.readFileSync('document.adoc', 'utf-8')); // <2>
console.log(html);
----
