...
The suite contains over 2,600 tests ported from the Asciidoctor Ruby test suite.
...
=== Running the Deno test suite

{uri-deno}[Deno] must be installed on your system before running these commands.
See the {uri-deno}[Deno website] for installation instructions.

To run the core test suite under Deno:
...
To run the CLI test suite under Deno:
...
Deno runs the same test files as the Node.js suite using `deno test` with the required permissions (`--allow-env`, `--allow-read`, `--allow-write`, `--allow-sys`, `--allow-run`, `--allow-net`).
