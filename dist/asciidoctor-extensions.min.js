...
v)}((d=!1!==(h=C(v,y))&&h!==c?h:(p=!1!==t&&t!==c)?C(v,t):t)===c||d.$$is_boolean&&1!=d)&&k.$raise(a.get("ArgumentError").$new("Invalid type for "+u+" extension: "+v));A=v.$new(n);A.$freeze();return f.get("ProcessorExtension").$new(b,A)}();return r.$config()["$[]"]("position")["$=="](">>")?x.$unshift(r):x["$<<"](r)});a.defn(b,"$add_syntax_processor",F=function(b,e){var d,g,q,m,h,k,p=F.$$p||c,r=c,u=c,x=c,y=u=c,t=x=c,A=c,n=c,u=c;F.$$p=null;r=b.$to_s().$tr("_"," ");u=(d=(g=r.$split(" ")).$map,d.$$p=(q=
...
this,e;null==a&&(a=c);return(e=a["$empty?"]())===c||e.$$is_boolean&&1!=e?(e=k["$const_defined?"](a))===c||e.$$is_boolean&&1!=e?d.$raise("Could not resolve class for name: "+b):k=k.$const_get(a):c},f.$$s=this,f),d).call(e);return k}),c)&&"class_for_name"})(a.get_singleton_class(B))})(r(K,"Asciidoctor").$$scope.base)}(a.base)})(Opal);(function(a){a.dynamic_require_severity="ignore";return a.nil})(Opal);
