...
In the following example, we define the `safe` option to `server` and we also define two attributes:
...
const html = await convert(content, { safe: 'server', attributes: { showtitle: true, icons: 'font' } })
console.log(html)
