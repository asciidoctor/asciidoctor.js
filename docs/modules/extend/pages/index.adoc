...
The https://asciidoctor.org/docs/user-manual/#builtin-attributes[Asciidoctor User Manual^] describes every built-in attribute that you can use.

Stylesheets::
As mentioned above, Asciidoctor provides a default stylesheet but you can xref:stylesheets/index.adoc#apply-theme[apply an alternative theme]. +
...
Custom converter::
A custom converter is a plain JavaScript class that implements the `convert(node, transform)` method.

Custom syntax highlighter::
A custom syntax highlighter controls how source blocks are rendered.
You can provide server-side or client-side highlighting by extending xref:syntax-highlighter/custom-syntax-highlighter.adoc[`SyntaxHighlighterBase`].
