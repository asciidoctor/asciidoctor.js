= Template Converter
:url-abstractnode-jsdoc: {url-jsdoc}/#abstractnode
:url-section-jsdoc: {url-jsdoc}/#section
:url-document-jsdoc: {url-jsdoc}/#document
...
Asciidoctor.js can fall back on a built-in converter.
...
await convertFile('doc.adoc', { safe: 'safe', backend: 'html5', template_dir: './templates' })
----
...
[source,js]
----
await convertFile('doc.adoc', { safe: 'safe', backend: 'html5', template_dirs: ['./templates-a', './templates-b'] })
----

=== Conflicts resolution

Here's how Asciidoctor.js resolves conflicts in the following situations:
...
If this file exists, Asciidoctor.js will load it and call the `configure` function if it's exported:
...
The `context` object will contain an <<isolated-environment,isolated environment>> (if supported) template engine keyed by template engine name:
...
With the above definition, the value `cdn` will be equal to:
...
export function getAssetUriScheme (document) {
  if (assetUriScheme) {
...
[source,js]
----
export default function ({ node, _, helpers }) {
...
We recommend reading the {url-jsdoc}[JS API documentation] to find out what it's available on each Node.
...
await convert(input, { template_cache: false })
----

It might be useful when you want to configure the same template directory with different options.
In the following example, we want to use an XML https://pugjs.org/language/doctype.html[doctype].
...
console.log(await convert(`image:cat.png[]`, options)) // <img src="cat.png"/>
console.log(await convert('image:cat.png[]', Object.assign(options, {
  template_engine_options: {
    pug: {
      doctype: 'xml'
    }
