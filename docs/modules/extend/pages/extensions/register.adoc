...
registryB.block(function () {
  ...
  self.onContext('paragraph')
  self.process(function (parent, reader) {
    // Replace period at end of line with three exclamation marks
...
Both registries have a `[shout]` block extension registered with a specific implementation.

The first block extension is registered in `registryA` and transforms the text to uppercase.
The other is registered in `registryB` and replaces `.` with `{nbsp}!!!`.