...
|`highlightjs-linenums-mode`
|`table`
|How line numbers are laid out when `linenums` is enabled: `table` (a gutter column) or `inline` (a number at the start of each line).
|===
...
- `linenums` adds line numbers (laid out per `highlightjs-linenums-mode`).
