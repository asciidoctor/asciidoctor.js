...
|`embed`
|`embed` reads the theme CSS from the installed highlight.js package and inlines it in a `<style>` tag (self-contained output). `link` emits a `<link>` to the CDN (or to `highlightjsdir`) instead.
